  ...
  return (
    <div className='w-full h-screen p-4 gap-3 flex flex-col items-center justify-center'>
      ...
      <form onSubmit={SubmitHandle} className='w-3/4 grid grid-cols-1 gap-3 py-3'>
          ...
          <input onChange={(e)=>setFName(e.target.value)} type="text" placeholder='Имя' className='w-full p-3 outline-0 text-xl' />
        </div>
          <div className='w-full flex outline-1 -outline-offset-1 rounded-xl  has-[input:focus-within]:outline-2 has-[input:focus-within]:-outline-offset-2 has-[input:focus-within]:outline-(--primary) duration-75'>
          <input onChange={(e)=>setLName(e.target.value)} type="text" placeholder='Фамилия' className='w-full p-3 outline-0 text-xl' />
        </div>
          <div className='w-full flex outline-1 -outline-offset-1 rounded-xl  has-[input:focus-within]:outline-2 has-[input:focus-within]:-outline-offset-2 has-[input:focus-within]:outline-(--primary) duration-75'>
          <input onChange={(e)=>setEmail(e.target.value)} type="text" placeholder='Почта' className='w-full p-3 outline-0 text-xl' />
        </div>
          <div className='w-full flex outline-1 -outline-offset-1 rounded-xl  has-[input:focus-within]:outline-2 has-[input:focus-within]:-outline-offset-2 has-[input:focus-within]:outline-(--primary) duration-75'>
          <input onChange={(e)=>setPassword(e.target.value)} type="password" placeholder='*******' className='w-full p-3 outline-0 text-xl' />
        </div>
